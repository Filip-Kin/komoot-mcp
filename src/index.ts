/**
 * komoot-mcp entrypoint.
 *
 * A single plain-HTTP origin (behind Zoraxy TLS) that is BOTH an OAuth 2.1
 * authorization-server facade (delegating login to Authelia) AND the protected
 * MCP resource. Claude apps connect as a remote custom connector. Structure and
 * OAuth machinery are lifted from home-bridge-mcp.
 */
...
server.listen(config.port, config.host, () => {
  ...
  if (config.devNoAuth) log("WARNING: KOMOOT_MCP_DEV_NO_AUTH=1 — auth is bypassed. Dev only.");
